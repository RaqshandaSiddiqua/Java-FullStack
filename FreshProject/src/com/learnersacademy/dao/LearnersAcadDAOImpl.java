...
public class LearnersAcadDAOImpl implements LearnersAcadDAO{
	...
	public boolean userLogin(String username, String password) throws MyException {
		...
		{
			String sql="select * from learnersacademy.tab_user where username=? and password=?";
			...
			if(result.next()) {
				...
				throw new MyException("Username and Password dont match"); 
			}
			
			
		} catch (ClassNotFoundException e) {
			throw new MyException("Internal Error contact sysadmin with error message "+e);
	...
	public boolean deleteSubject(Integer subjectID) throws MyException {
		...
		if(c!=1) {
			throw new MyException("Deletion Failed");
		}
		else {
			return true;
		}
	...
	public boolean addSubject(SubjectObj subject) throws MyException {
		...
		int c=preparedStatement.executeUpdate();
		if(c!=1) {
			throw new MyException("Insertion Failed");
		}
		else {
			return true;
		}
	...
	public boolean updSubject(SubjectObj subject) throws MyException {
		try (Connection connection = DBConnection.getConnection()) {
			...
			throw new MyException("Internal error occured kindly contact sysadmin with error message "+e);
		}
		return false;
		}
